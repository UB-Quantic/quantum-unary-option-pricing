...
from unary_ideal_tests import log_normal
...
def binary_quantum_sim(qu, S0, sig, r, T):
    #samples = 2 ** qu
    ...
    dev = BasicAer.get_backend('qasm_simulator')

    job = execute(qc, dev, shots=shots)
    result = job.result()
...
def binary_benchmark_sim(qu, S0, sig, r, T):
    (S, ln, prob_sim), (mu, mean, variance) = binary_quantum_sim(qu, S0, sig, r, T)
    ...
    ax.plot(Sp, (S[1] - S[0]) * lnp / (Sp[1] - Sp[0]), 'C1')
    ...
    plt.title('Option price distribution for {} qubits - binary'.format(qu))
    ax.legend()
    fig.tight_layout()
    text='qubits_{}_prob_2.png'.format(qu)
    fig.savefig(text)
...
def binary_qu_cl(qu, S0, sig, r, T, K, cl_payoff):
