...
def binary_quantum_sim(qu, S0, sig, r, T, noise_objects):
    ...
    job = execute(qc, dev, shots=shots, basis_gates=basis_gates, noise_model=noise_model, coupling_map=coupling_map, optimization_level=3)
    result = job.result()
...
def binary_benchmark_sim(qu, S0, sig, r, T, noise_objects):
...
def payoff_quantum_sim(qu, S0, sig, r, T, K, noise_objects, printer=True):
    ...
    error = np.abs(100 * (cl_payoff - qu_payoff) * 2 / (cl_payoff + qu_payoff))
    if printer==True:
        ...
        print('Quantum Payoff: {}'.format(qu_payoff))
        print('')
        print('Percentage off: {}%'.format(error))
    return qu_payoff, cl_payoff, error
