...
from noise_mapping import unary_coupling
...
def unary_benchmark_sim(qu, S0, sig, r, T, noise_objects, all_samples=False):
    ...
    if all_samples:
        ...
        ax.bar(Sp, prob_sim, label='Samples')
        un_samples = [prob_sim[s] for s in S_bar]
        ax.bar(S_bar, un_samples, width=S_bar/20, label='Desired samples', color='black')
        ax.scatter(S_bar, ln, s=750, color='C1', label='Exact', marker='_')
        ...
        ax.set(xscale='log')
        fig.tight_layout()
        fig.savefig('unary/qubits:{}_S0:{}_sig:{}_r:{}_T:{}_error:{}_all:.png'.format(qu, S0, sig, r, T, noise_objects[3]))
        ...
        ax.scatter(S, ln, s=750, color='C1', label='Exact', marker='_')
        ...
        ax.legend()
        fig.tight_layout()
        fig.savefig('unary/qubits:{}_S0:{}_sig:{}_r:{}_T:{}_error:{}:.png'.format(qu, S0, sig, r, T, noise_objects[3]))


def unary_quantum_dev(qu, S0, sig, r, T):
    '''
    Load your IBM account and get the backend for the 14 qubit device
    ...
    transpiled_circ = transpile(circ_prob, backend=backend_device, optimization_level=3)
    print('Depth:', transpiled_circ.depth())

    plot_circuit_layout(transpiled_circ, backend_device)

    job_dev = execute(transpiled_circ, backend_device, shots=samples)
    job_monitor(job_dev)
    result_dev = job_dev.result()  #Run the test circuit through the quantum computer and sample the results
                                        #in order to get the estimated probabilities

    counts_dev = result_dev.get_counts(transpiled_circ)
    prob_dev = extract_probability(qu, counts_dev, samples)
    prob_dev_2 = extract_probability_dev(qu, counts_dev, samples)

    return (S, prob_dev), (mu, mean, variance)


def unary_benchmark_dev(qu, S0, sig, r, T):
    (S, prob_dev), (mu, mean, variance) = unary_quantum_dev(qu, S0, sig, r, T)
    Sp = np.linspace(mean-3*np.sqrt(variance), mean+3*np.sqrt(variance), 2**qu) #Generate a the exact target distribution to benchmark against quantum results
    lnp = log_normal(Sp, mu, sig, T)
    ...
    fig, ax = plt.subplots()
    rects2 = ax.bar(S, prob_dev, width, label='Quantum')
    ax.plot(Sp, max(prob_dev) * lnp / max(lnp), 'C1', label='Exact')
    #ax.vlines(K, 0, max(prob_dev), linestyles='dashed', label='K = {}'.format(K))  # Strike price marker
    plt.ylabel('Probability')
    plt.xlabel('Option price')
...
def payoff_circuit(qubits, K, S):
    ...
    for i in range(qK, qubits):                              #Control-RY rotations controled by states
        angle = 2 * np.arcsin(np.sqrt((S[i]-K)/S[qubits-1])) #with higher value than the strike
...
def payoff_quantum_sim(qu, S0, sig, r, T, K, noise_objects):
    ...
    qu_payoff_sim = counts_payoff_sim.get('1') * S[qu - 1] / samples
...
def unary_qu_cl(qu, S0, sig, r, T, K, noise_objects):
    samples=2**qu
